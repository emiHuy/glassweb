...
def build_report_html(scan_data: dict) -> str:
    ...
    for entity, counts in entity_counts.items():
        ...
        <tr>
            ...
            <td class="type-cell">{counts["get"]}</td>
            <td class="type-cell">{counts["post"]}</td>
            <td class="type-cell">{", ".join(counts["categories"])}</td>
        </tr>""")
    ...
    return f"""
        ...
        {"" if not entity_rows else f'''
            ...
            <table>
                <thead><tr><th>Entity</th><th>Domains</th><th>Requests</th><th>GET</th><th>POST</th><th>Categories</th></tr></thead>
